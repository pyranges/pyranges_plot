...
def _add_source_figure(master, source, row, visible):
    ...
    for ann in source.layout.annotations or []:
        ...
        if ann_dict.get("xref") == "paper" or ann_dict.get("yref") == "paper":
            continue
...
def _zip_summary(panel, id_col=None):
    ...
    label = panel.label if panel.chrom is not None else f"Track {panel.data_index + 1}"
...
def browser(
    ...
    **kwargs,
    ...
    Each panel gets a compact dropdown button. The control changes only that panel,
    so panels can independently use ``zip``, ``squish``, ``packed`` or ``full``
    views. ``list`` is accepted as a deprecated alias for ``full``.
    """
    ...
    for row_ix, panel in enumerate(panels, start=1):
        ...
        for mode in modes:
            ...
            if y_vals:
                ...
                mode_ranges[mode] = [ymin - ypad, ymax + ypad]
            else:
                mode_ranges[mode] = [0, 1]
            all_x.extend(x_vals)
            all_y.extend(y_vals)
        ...
        mode_trace["__ranges__"] = mode_ranges
        ...
        master.update_yaxes(
            range=mode_ranges[default_mode],
    ...
    for panel_ix, panel in enumerate(panels):
        ...
        for mode in modes:
            ...
            layout_update[f"{yaxis_name}.range"] = panel_mode_trace_indices[panel_ix][
                "__ranges__"
            ][mode]
            buttons.append(
                dict(
